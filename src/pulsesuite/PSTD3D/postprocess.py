...
def organize_output(base_dir="dataQW", wire=1):
    ...
    if os.path.isfile(info_file):
        ...
        for name, col_idx in _INFO_COLUMNS.items():
            if col_idx < data.shape[1]:
                ...
                np.savetxt(
                    outpath, np.column_stack([time_col, data[:, col_idx]]), fmt="%.6e"
                )
    ...
    if os.path.isfile(info_file_path):
        ...
        if data.shape[1] > 6:
            ...
            np.savetxt(outpath, np.column_stack([time_col, total_energy]), fmt="%.6e")
            print("  -> derived En.t.dat (total energy)")
