...
def _nyquist_1D(F):
    """
    Multiply every even-indexed element by -1 (i.e. (-1)^n pattern).

    This is a half-sample shift in DFT space, equivalent to fftshift.
    Matches Fortran nyquist_1D in fftw.F90.
    Uses 1-based parity to match Fortran: element at 0-based index i
    corresponds to Fortran index i+1, so we flip when (i+1) is even,
    i.e. when i is odd.
    """
    F[1::2] = -F[1::2]
...
def FFTG(F):
    """
    FFT with Gaussian normalization (centered).

    Performs centered FFT: nyquist(F) -> fft(F) -> nyquist(F) -> F = -F/Nf
    Matches Fortran chain: FFTG -> FFTC -> nyquist + fft + nyquist
    ...
    """
    Nf = len(F)
    # OLD: missing nyquist calls
    # F[:] = pyfftw.interfaces.numpy_fft.fft(F)
    # F[:] = -F[:] / (Nf * 1.0)

    # NEW: matches Fortran FFTC = nyquist -> fft -> nyquist
    _nyquist_1D(F)
    F[:] = pyfftw.interfaces.numpy_fft.fft(F)
    _nyquist_1D(F)
    F[:] = -F[:] / (Nf * 1.0)
...
def iFFTG(F):
    """
    IFFT with Gaussian normalization (centered).

    Performs centered IFFT: nyquist(F) -> ifft(F) -> nyquist(F) -> F = -F*Nf
    Matches Fortran chain: iFFTG -> iFFTC -> nyquist + ifft + nyquist
    ...
    # NEW: matches Fortran iFFTC = nyquist -> ifft -> nyquist
    _nyquist_1D(F)
    F[:] = pyfftw.interfaces.numpy_fft.ifft(F)
    _nyquist_1D(F)
    F[:] = -F[:] * Nf
