...
def QuantumWire(
    ...
    if DoQWP[0] and _propagate:
        ...
        # ── RhoBAnalytic: product-rule -div(P) with analytical gate derivatives ──
        # Matches Fortran rhoPJ.f90:328-352
        # P_3D = P_1D(x) * gx(x) * gy(y) * gz(z)
        # div(P) = dPx/dx * gx*gy*gz + Py * gx*dgy/dy*gz + Pz * gx*gy*dgz/dz
        dx = GetDx(space)
        Pxavg = np.sum(Pxx, axis=1) / float(_Nw)
        Pyavg = np.sum(Pyy, axis=1) / float(_Nw)
        Pzavg = np.sum(Pzz, axis=1) / float(_Nw)

        # Analytical gate derivatives: dgy/dy = -(y-y0)/ay^2 * gy
        ...
        # Term 1: -dPx/dx * gx * gy * gz
        RhoBterm = np.zeros((Nx, Ny, Nz), dtype=np.complex128)
        QWPlacement(dPxdx, gx, gy, gz, RhoBAnalytic)
        RhoBAnalytic[:] = -RhoBAnalytic

        # Term 2: -Py * gx * dgy/dy * gz
        QWPlacement(Pyavg, gx, dgy, gz, RhoBterm)
        RhoBAnalytic[:] = RhoBAnalytic - RhoBterm

        # Term 3: -Pz * gx * gy * dgz/dz
        QWPlacement(Pzavg, gx, gy, dgz, RhoBterm)
        RhoBAnalytic[:] = RhoBAnalytic - RhoBterm

        # ── RhoBSmooth: Gaussian-filtered versions of both ──
        # sigma = 2*dx removes grid-scale artifacts from derivatives and gate discretization
        sigma_x = 2.0 * GetDx(space)
        sigma_y = 2.0 * GetDy(space)
        sigma_z = 2.0 * GetDz(space)
        qx_arr = GetKxArray(space)
        ...
        RhoBSmooth_k *= filt
        RhoBSmooth[:] = pyfftw.interfaces.numpy_fft.ifftn(RhoBSmooth_k)

        # Smooth analytical
        RhoBAnSmooth_k = pyfftw.interfaces.numpy_fft.fftn(RhoBAnalytic.copy())
        RhoBAnSmooth_k *= filt
        RhoBAnSmooth[:] = pyfftw.interfaces.numpy_fft.ifftn(RhoBAnSmooth_k)
    ...
    for k in prange(Nz):  # pylint: disable=not-an-iterable
        for j in range(Ny):
            for i in range(Nx):
                ...
                # RhoB_k[i, j, k] = -ii_val * qdotP  # OLD: ρ_bound(k) = -i k·P(k)
...
def ElongfromRho(
        ...
        for k in range(Nz):
            for j in range(Ny):
                for i in range(Nx):
                    ...
                    # RhoB_k[i, j, k] = -ii * qdotP  # OLD: ρ_bound(k) = -i k·P(k)
    ...
    RhoBSpec[:] = RhoB_k
    RhoBSpec[:] = pyfftw.interfaces.numpy_fft.ifftn(RhoBSpec)
...
class QWArray:
    ...
    def QuantumWire(
        ...
        if DoQWP[0] and self.propagate:
            ...
            # ── RhoBAnalytic: product-rule -div(P) ──
            dx_val = GetDx(space)
            Pxavg = np.sum(Pxx, axis=1) / float(self.Nw)
            Pyavg = np.sum(Pyy, axis=1) / float(self.Nw)
            Pzavg = np.sum(Pzz, axis=1) / float(self.Nw)
            ...
            # ── RhoBSmooth / RhoBAnSmooth: Gaussian-filtered versions ──
            sigma_x = 2.0 * GetDx(space)
            sigma_y = 2.0 * GetDy(space)
            sigma_z = 2.0 * GetDz(space)
            qx_arr = GetKxArray(space)
